...
/** Дорама бетіндегі үзінді ойнатқышы */
function trailerHTML(d) {
  const t = parseTrailer(d.trailer);
  if (!t) return "";

  // Танылмаған сілтеме — жай сілтеме беріп қоямыз
  if (t.kind === "link")
    return `<p style="margin-top:22px">
      <a class="btn btn--light" href="${esc(t.src)}" target="_blank" rel="noopener">Үзіндіні көру</a>
    </p>`;

  const body = {
    youtube: `<iframe src="https://www.youtube-nocookie.com/embed/${esc(t.id)}?rel=0"
    ...
    file: `<video src="${esc(t.src || "")}" controls preload="metadata" playsinline
                poster="${esc(Img.url(d.poster))}"
                controlsList="nodownload" oncontextmenu="return false"></video>`
  }[t.kind];

  return `
    <section class="trailer ${t.kind === "tiktok" ? "trailer--tall" : ""}">
      <h3>Үзінді</h3>
      <div class="trailer__box">${body}</div>
    </section>`;
}
