...
const Publish = {
  ...
  /** Екілік файлды (видео, сурет) репозиторийге жүктеу */
  async putBinary(path, file, onStep = () => {}) {
    onStep("Файл оқылуда…");
    ...
    // Файл бұрыннан бар болса, оның sha-сы керек
    let sha;
    try { sha = (await this.getFile2(path)).sha; } catch {}

    onStep("GitHub-қа жүктелуде…");
    const c = this.cfg();
    await this.api(`contents/${path}`, {
      method: "PUT",
      body: JSON.stringify({
        message: `Файл жүктелді: ${path}`,
  ...
  /** Тек sha керек болғанда (мазмұнын оқымаймыз — видео үлкен) */
  async getFile2(path) {
    const c = this.cfg();
    const data = await this.api(`contents/${path}?ref=${c.branch}`);
    return { sha: data.sha };
